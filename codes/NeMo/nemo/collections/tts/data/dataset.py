...
class TTSDataset(Dataset):
            ...
            with open(Path(manifest_file).expanduser(), 'r') as f:
                ...
                for line in tqdm(f):
                    ...
                    file_info = {
                        ...
                        'phoneme_length_sequence': item['phoneme_length'] if 'phoneme_length' in item else None,
                    }
    ...
    def add_emotions(self, **kwargs):
        self.emotion_path = kwargs.pop('emotion_path', None)
    ...
    def __getitem__(self, index):
        ...
        if (
            ...
            audio, audio_length = features, torch.tensor(features.shape[0]).long()
        ##add phoneme_length_sequence
        if "phoneme_length_sequence" in sample: #!!!!!!!!!
            phoneme_length_sequence = torch.tensor(sample['phoneme_length_sequence']).long()
        else:
            phoneme_length_sequence = None
        ...
        emotion = None
        if Emotions in self.sup_data_types_set:
            _path = os.path.join('./emotion_embedding_npy',sample['emotion']+'.npy')
            emotion = torch.from_numpy(np.load(_path).T) # C, T
    ...
    def general_collate_fn(self, batch):
        ...
        for i, sample_tuple in enumerate(batch):
            ...
            if phoneme_length_sequence is not None:
                phoneme_length_sequence = general_padding(phoneme_length_sequence, len(phoneme_length_sequence), max_tokens_len)
                phoneme_length_sequences.append(phoneme_length_sequence)
        ...
        data_dict = {
            ...
            "phoneme_length_sequences":torch.stack(phoneme_length_sequences),
            ...
            "emotions": torch.stack(emotions) if Emotions in self.sup_data_types_set else None,
