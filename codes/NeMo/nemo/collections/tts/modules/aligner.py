...
class AlignmentEncoder(torch.nn.Module):
    ...
    def get_cosine_dist(self, queries_enc, keys_enc):
    ...
    def get_rbf_dist(self, queries_enc, keys_enc, sigma=1.0):
        queries_enc = rearrange(queries_enc, "B C T1 -> B C T1 1")
        keys_enc = rearrange(keys_enc, "B C T2 -> B C 1 T2")
        squared_dist = torch.sum((queries_enc - keys_enc) ** 2, dim=1, keepdim=True)
        # RBF核直接生成相似度，不需要再取负号
        rbf_similarity = torch.exp(-squared_dist / (2 * sigma ** 2))
        # 转换为距离形式
    ...
    def forward(self, queries, keys, mask=None, attn_prior=None, conditioning=None):
        ...
        # Cosine distance
        distance = self.get_cosine_dist(queries_enc,keys_enc)
        self._apply_mask(distance, mask, float("inf"))
